...
class SplitBarrierStrategyTest {
    ...
    void noBarrier_readToRead_sameQueue() {
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            strategy.emit(buf, consumer, 0, batch, arena);
    ...
    void emitsBarrier_writeToRead() {
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            assertEquals(0x00000800, batch.srcStage(0)); // COMPUTE
            assertEquals(0x00000080, batch.dstStage(0)); // FRAGMENT
        }
    }
    ...
    void noBarrier_firstUseWithNoHistory() {
        ...
        buf.updateState(0, 0, VK_QUEUE_FAMILY_IGNORED);
    ...
    void emitsImageBarrier_layoutTransition() {
        ...
        img.updateState(0, 0x00000001, VK_QUEUE_FAMILY_IGNORED); // TOP_OF_PIPE
        ...
        try (Arena arena = Arena.ofConfined()) {
            BarrierBatch batch = new BarrierBatch();
            strategy.emit(img, consumer, 0, batch, arena);
    ...
    void batchAccumulatesMultipleBarriers() {
        try (Arena arena = Arena.ofConfined()) {
            ...
            strategy.emit(buf1, ResourceEdge.read(buf1, 0x00000020, 0x00000080), 0, batch, arena);

            GraphResource buf2 = TestResources.transientBuffer("buf2");
            buf2.updateState(0x00001000, 0x00001000, 0); // TRANSFER_WRITE, TRANSFER
            strategy.emit(buf2, ResourceEdge.read(buf2, 0x00000020, 0x00000080), 0, batch, arena);

            assertEquals(2, batch.count());
            // Per-barrier stage masks
            assertEquals(0x00000800, batch.srcStage(0)); // COMPUTE
            assertEquals(0x00001000, batch.srcStage(1)); // TRANSFER
            assertEquals(0x00000080, batch.dstStage(0)); // FRAGMENT
            assertEquals(0x00000080, batch.dstStage(1)); // FRAGMENT
            // Combined masks
            assertEquals(0x00000800 | 0x00001000, batch.combinedSrcStageMask());
            assertEquals(0x00000080, batch.combinedDstStageMask());
        }
    }
    ...
    void emitsOwnershipTransfer_bufferCrossQueue() {
        ...
        try (Arena arena = Arena.ofConfined()) {
            BarrierBatch batch = new BarrierBatch();
            strategy.emit(buf, consumer, 1, batch, arena); // consumer on queue family 1
    ...
    void emitsOwnershipTransfer_imageCrossQueue_withLayoutTransition() {
        ...
        try (Arena arena = Arena.ofConfined()) {
            BarrierBatch batch = new BarrierBatch();
            strategy.emit(img, consumer, 1, batch, arena); // consumer on queue family 1
    ...
    void noOwnershipTransfer_sameQueueFamily() {
        ...
        try (Arena arena = Arena.ofConfined()) {
            BarrierBatch batch = new BarrierBatch();
            strategy.emit(buf, consumer, 0, batch, arena); // same queue family
    ...
    void noOwnershipTransfer_ignoredQueueFamily() {
        ...
        buf.updateState(0x00000040, 0x00000800, VK_QUEUE_FAMILY_IGNORED); // SHADER_WRITE, COMPUTE, QUEUE_FAMILY_IGNORED

        ResourceEdge consumer = ResourceEdge.read(buf, 0x00000020, 0x00000080);

        try (Arena arena = Arena.ofConfined()) {
            BarrierBatch batch = new BarrierBatch();
            strategy.emit(buf, consumer, 1, batch, arena);
