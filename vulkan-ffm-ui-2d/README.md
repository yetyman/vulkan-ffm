...
A 2D UI scene graph built on the `vulkan-ffm-foundation` ECS node tree.
...
- **Nine-slice rectangle rendering** — bulk-rendered via a single draw call using the
  ECS TraversalView + dense vertex buffer pattern
...
This module uses `vulkan-ffm-foundation`'s ECS (`Tree`, `Node`, `Component`, `TraversalView`)
...
```
vulkan-ffm-ui-2d
  └── vulkan-ffm-foundation (ECS, UILayer, vulkan-core)
```
