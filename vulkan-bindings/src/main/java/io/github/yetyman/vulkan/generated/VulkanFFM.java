...
public class VulkanFFM extends VulkanFFM_1 {
    ...
    // NOTE: Hand-supplemented, not jextract-generated. vkCmdDrawIndexedIndirectCount is core
    // Vulkan 1.2 API (declared plainly in vulkan_core.h with no extension guard) but was absent
    // from jextract's output for reasons not diagnosed; DynFunctionGenerator's vk.xml-driven
    // fallback did not pick it up either since it scans only for functions jextract missed by
    // name, and this entry follows the exact structural pattern jextract uses for the sibling
    // vkCmdDrawMeshTasksIndirectCountEXT above (same six-parameter shape: buffer, offset,
    // countBuffer, countBufferOffset, maxDrawCount, stride).
    //
    // Added manually because the mesh module's GPU-driven rendering samples (see
    // plans/mesh/09-roadmap.md Phase 5 and vulkan-ffm-mesh's IndirectDrawEncoder) need this call
    // and it was not available any other way at the time. Those plan documents are a point-in-time
    // record and are not guaranteed to still exist by the time anyone reads this comment, so the
    // reasoning is restated here rather than only linked.
    //
    // Self-diagnosing by design: if vulkan-bindings is ever regenerated and jextract picks this
    // symbol up on its own (a jextract version fix, a header change, whatever the reason), this
    // whole class plus the four static methods below it will collide with jextract's freshly
    // generated versions of the same names in the same file, and the module will fail to compile
    // with a duplicate-class/duplicate-method error pointing straight at this block. That failure
    // is the intended signal to delete this hand-written block and keep the regenerated one - no
    // separate tracking issue, changelog entry, or tribal knowledge required to know why it broke
    ...
    // NOTE: Hand-supplemented, same reasoning as vkCmdDrawIndexedIndirectCount above. Core
    // Vulkan 1.0 API (vulkan_core.h line ~4868), missing from jextract's output for the same
    // undiagnosed reason. Needed to zero a draw-count buffer on the GPU each frame before a
    // culling compute dispatch writes a fresh count via atomicAdd, since a compute shader cannot
    // safely reset its own output counter across workgroup boundaries (barrier() only
    // synchronizes within one workgroup). Same self-diagnosing duplicate-symbol behavior applies
    // if vulkan-bindings is regenerated and jextract picks this up on its own.
