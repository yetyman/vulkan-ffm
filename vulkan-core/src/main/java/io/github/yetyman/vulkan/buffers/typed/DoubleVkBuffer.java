...
/**
 * Typed buffer view for {@code double} data.
 *
...
public class DoubleVkBuffer implements AutoCloseable {
    ...
    public DoubleVkBuffer(IBuffer buffer, int count) {
        ...
        this.mirror = (buffer instanceof ManagedBuffer mb && mb.observability() instanceof MirrorCapable mc)
                ? mc.mirrorMemory().asByteBuffer().asDoubleBuffer() : null;
    }
    ...
    public void write(double[] data, int startIndex, VkQueue queue) {
        writeRange(data, 0, data.length, startIndex, queue);
    }

    public GpuCompletion writeAsync(double[] data, int startIndex, VkQueue queue) {
     ...
     */
    public void writeRange(double[] src, int srcOffset, int count, int dstIndex, VkQueue queue) {
    ...
    public GpuCompletion writeRangeAsync(double[] src, int srcOffset, int count, int dstIndex, VkQueue queue) {
        checkRange(dstIndex, count);
        BufferWriteScope scope = buffer.acquireWrite((long) dstIndex * STRIDE, (long) count * STRIDE, queue);
        MemorySegment.copy(src, srcOffset, scope.segment(), JAVA_DOUBLE_UNALIGNED, 0, count);
        scope.close();
     ...
     */
    public GpuCompletion writeStridedAsync(double[] src, int srcOffset, int componentsPerElement, int count,
        ...
        for (int e = 0; e < count; e++) {
            ...
            for (int c = 0; c < componentsPerElement; c++) {
                dst.set(JAVA_DOUBLE_UNALIGNED, base + (long) c * STRIDE, src[s++]);
            }
        }
        scope.close();
        return scope.completion();
    ...
    public void writeStrided(double[] src, int srcOffset, int componentsPerElement, int count,
                             int dstElementIndex, long dstStrideBytes, VkQueue queue) {
     ...
     */
    public void readRange(int startIndex, int length, double[] dst, int dstOffset) {
        ...
        try (BufferReadScope scope = buffer.acquireRead((long) startIndex * STRIDE, (long) length * STRIDE, null)) {
            MemorySegment.copy(scope.segment(), JAVA_DOUBLE_UNALIGNED, 0, dst, dstOffset, length);
        }
    }
