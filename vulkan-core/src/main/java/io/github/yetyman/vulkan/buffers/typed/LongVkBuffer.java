...
/**
 * Typed buffer view for {@code long} data.
 *
...
public class LongVkBuffer implements AutoCloseable {
    ...
    public LongVkBuffer(IBuffer buffer, int count) {
        ...
        this.mirror = (buffer instanceof ManagedBuffer mb && mb.observability() instanceof MirrorCapable mc)
                ? mc.mirrorMemory().asByteBuffer().asLongBuffer() : null;
    }
    ...
    public GpuCompletion writeAsync(long[] data, int startIndex, VkQueue queue) {
     ...
     */
    public void writeRange(long[] src, int srcOffset, int count, int dstIndex, VkQueue queue) {
    ...
    public GpuCompletion writeRangeAsync(long[] src, int srcOffset, int count, int dstIndex, VkQueue queue) {
        checkRange(dstIndex, count);
        BufferWriteScope scope = buffer.acquireWrite((long) dstIndex * STRIDE, (long) count * STRIDE, queue);
        MemorySegment.copy(src, srcOffset, scope.segment(), JAVA_LONG_UNALIGNED, 0, count);
        scope.close();
     ...
     */
    public GpuCompletion writeStridedAsync(long[] src, int srcOffset, int componentsPerElement, int count,
        ...
        for (int e = 0; e < count; e++) {
            ...
            for (int c = 0; c < componentsPerElement; c++) {
                dst.set(JAVA_LONG_UNALIGNED, base + (long) c * STRIDE, src[s++]);
            }
        }
        scope.close();
        return scope.completion();
    ...
    public void writeStrided(long[] src, int srcOffset, int componentsPerElement, int count,
                             int dstElementIndex, long dstStrideBytes, VkQueue queue) {
     ...
     */
    public void readRange(int startIndex, int length, long[] dst, int dstOffset) {
        ...
        try (BufferReadScope scope = buffer.acquireRead((long) startIndex * STRIDE, (long) length * STRIDE, null)) {
            MemorySegment.copy(scope.segment(), JAVA_LONG_UNALIGNED, 0, dst, dstOffset, length);
        }
    }
