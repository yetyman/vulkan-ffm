...
/**
 * GraphicsFrame driving a UIComposite with multiple layers (Scene3DOverlayLayer +
 * GPUDrivenTextLayer + HoverHighlightLayer) through the direct-render path in one command
 * buffer. Each layer binds its own pipeline/descriptor state before drawing - UIComposite.render()
 * simply calls each layer's render() in ascending order() (lowest/background first), so there is
 * no shared pipeline state to coordinate between layers.
 */
public class MultiLayerExampleFrame extends GraphicsFrame {

    private final UIComposite composite;
    private long frameNumber = 0;

    public MultiLayerExampleFrame(java.lang.foreign.Arena arena, VkDevice device, VkQueue queue,
